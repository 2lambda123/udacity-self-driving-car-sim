...
public class CommandServer : MonoBehaviour {
  ...
  void OnSteer(SocketIOEvent obj) {
    ...
    CarRemoteControl.SteeringAngle =
        float.Parse(jsonObject.GetField("steering_angle").ToString());
    CarRemoteControl.Acceleration =
        float.Parse(jsonObject.GetField("throttle").ToString());
  ...
  void EmitTelemetry(SocketIOEvent obj) {
    UnityMainThreadDispatcher.Instance().Enqueue(() => {
      ...
      if ((Input.GetKey(KeyCode.W)) || (Input.GetKey(KeyCode.S))) {
        ...
        data["image"] = Convert.ToBase64String(
            CameraHelper.CaptureFrame(FrontFacingCamera));
