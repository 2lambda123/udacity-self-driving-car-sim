...
public class CarController : MonoBehaviour {
  ...
  public bool IsRecording {
    ...
    set {
      ...
      if (value == true) {
        ...
        // save the cars coordinate parameters so we can reset it to this
        // properly after capturing data
        saved_position = transform.position;
        saved_rotation = transform.rotation;
        // see how many samples we captured use this to show save percentage in
        // UISystem script
  ...
  public bool checkSaveLocation() {
    if (m_saveLocation != "") {
      ...
      SimpleFileBrowser.ShowSaveDialog(OpenFolder, null, true, null,
  ...
  private void CalculateGearFactor() {
    ...
    // gear factor is a normalised representation of the current speed within
    // the current gear's range of speeds. We smooth towards the 'target' gear
    // factor, so that revs don't instantly snap up or down when changing gear.
    var targetGearFactor = Mathf.InverseLerp(
        f * m_GearNum, f * (m_GearNum + 1), Mathf.Abs(CurrentSpeed / MaxSpeed));
    m_GearFactor =
  ...
  private void CalculateRevs() {
    ...
    // (this is done in retrospect - revs are not used in force/power
    // calculations)
    CalculateGearFactor();
    var gearNumFactor = m_GearNum / (float)NoOfGears;
    var revsRangeMin =
        ULerp(0f, m_RevRangeBoundary, CurveFactor(gearNumFactor));
  ...
  private void CapSpeed() {
    ...
    case SpeedType.MPH:
      ...
      if (speed > m_Topspeed)
        m_Rigidbody.velocity =
            (m_Topspeed / 2.23693629f) * m_Rigidbody.velocity.normalized;
      break;
    ...
    case SpeedType.KPH:
      ...
      if (speed > m_Topspeed)
        m_Rigidbody.velocity =
            (m_Topspeed / 3.6f) * m_Rigidbody.velocity.normalized;
      break;
    }
  }
  ...
  private void SteerHelper() {
    for (int i = 0; i < 4; i++) {
      ...
      if (wheelhit.normal == Vector3.zero)
        return; // wheels arent on the ground so dont realign the rigidbody
                // velocity
    }

    // this if is needed to avoid gimbal lock problems that will make the car
  ...
  // crude traction control that reduces the power to wheel if the car is wheel
  // spinning too much
  ...
  // Changed the WriteSamplesToDisk to a IEnumerator method that plays back
  // recording along with percent status from UISystem script instead of showing
  // frozen screen until all data is recorded
  public IEnumerator WriteSamplesToDisk() {
    yield return new WaitForSeconds(
        0.000f); // retrieve as fast as we can but still allow communication of
                 // main thread to screen and UISystem
    if (carSamples.Count > 0) {
      ...
      string row = string.Format("{0},{1},{2},{3},{4},{5},{6}\n", centerPath,
                                 leftPath, rightPath, sample.steeringAngle,
                                 sample.throttle, sample.brake, sample.speed);
    ...
    if (carSamples.Count > 0) {
      ...
      // otherwise sometimes the car ended up in strange areas
  ...
  public bool getSaveStatus() { return isSaving; }
  ...
  public IEnumerator Sample() {
    ...
    if (m_saveLocation != "") {
      ...
      sample.timeStamp =
          System.DateTime.Now.ToString("yyyy_MM_dd_HH_mm_ss_fff");
  ...
  private string WriteImage(Camera camera, string prepend, string timestamp) {
    ...
    texture2D.ReadPixels(
        new Rect(0, 0, targetTexture.width, targetTexture.height), 0, 0);
    texture2D.Apply();
