...
public class WaypointTracker {
  ...
  private int NextWaypoint(CarController cc) {
    ...
    // It should be a small value, if it's large then it means we have to turn
    // around and the waypoint should be actually be the next one.
    float angle = Quaternion.Angle(cc.transform.rotation,
  ...
  public float CrossTrackError(CarController cc) {
    ...
    // Debug.Log(string.Format("Progress between waypoints {0} and {1} = {2}",
    // prev_wp, next_wp, t)); Cross track error
    var threshold = 0.05f;
    if (t >= (1 - threshold)) {
      var p0 = prev_wp;
      var p1 = next_wp;
      ...
      var bp = BezierPoint(v0, v1, v2, newt) -
               new Vector2(waypoints[p0].x, waypoints[p0].z);
      // Debug.Log(string.Format("Bezier point {0}, Actual projection {1},
      // Position {2} {3} {4}", bp, proj, x0, t, newt));
      proj = bp;
    } else if (t <= threshold) {
      var p2 = next_wp;
      var p1 = prev_wp;
      ...
      var bp = BezierPoint(v0, v1, v2, newt) -
               new Vector2(waypoints[p1].x, waypoints[p1].z);
      // Debug.Log(string.Format("Bezier point {0}, Actual projection {1},
      // Position {2} {3} {4}", bp, proj, x0, t, newt));
      proj = bp;
    }
    ...
    // If projected position is closer is means the vehicle is to the right of
    // the line hence the CTE will be negative (turn left). If the vehicle
    // position is closer is means the vehicle is to the left of the line hence
    // the CTE will be positive (turn right).
